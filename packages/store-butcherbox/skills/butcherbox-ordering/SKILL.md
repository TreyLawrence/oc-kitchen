...
## How ButcherBox differs from Instacart/Weee!
...
If the status is `"past"`, inform the user the cutoff has passed and those proteins will need to come from Instacart instead.
...
2. **Align with meal plans.** When generating a grocery list, identify proteins that ButcherBox carries (chicken, beef, pork, salmon, etc.) and suggest adding them to the upcoming box instead of buying from Instacart. "Your meal plan needs 4 lbs of chicken thighs — want to add those to your ButcherBox instead of ordering on Instacart? They'll arrive Thursday."
3. **Don't duplicate.** If protein is coming from ButcherBox, remove it from the Instacart grocery list.
